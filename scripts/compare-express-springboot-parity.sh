...
# Replace {{seed_1}} and {{seed_2}} placeholders with the real IDs
# created during the deterministic seed step.
...
# Extract the numeric "id" field from the simple todo JSON response.
extract_json_id() {
  local json="$1"
...
compare_case() {
  ...
  local springboot_prefix="${TMP_DIR}/${safe_name}_springboot"
  ...
  perform_request "${name} - Spring Boot" "${method}" "${SPRINGBOOT_BASE_URL}${springboot_path}" "${body}" "${springboot_prefix}"
  ...
  local springboot_headers_norm="${springboot_prefix}.headers.norm"
  local express_body_norm="${express_prefix}.body.norm"
  local springboot_body_norm="${springboot_prefix}.body.norm"

  normalize_headers "${express_prefix}.headers" > "${express_headers_norm}"
  normalize_headers "${springboot_prefix}.headers" > "${springboot_headers_norm}"

  normalize_body "${express_prefix}.body" > "${express_body_norm}"
  normalize_body "${springboot_prefix}.body" > "${springboot_body_norm}"

  echo "-- Comparison --"

  if [[ "${express_status_code}" == "${springboot_status_code}" ]]; then
...
seed_test_data() {
  ...
  local springboot_seed_1_json
  local springboot_seed_2_json
  ...
  springboot_seed_1_json="$(
    curl -sS -X POST "${SPRINGBOOT_BASE_URL}/todos" \
      -H "Content-Type: application/json" \
      -d '{"title":"seed-one","completed":false,"order":1}'
  )"

  springboot_seed_2_json="$(
    curl -sS -X POST "${SPRINGBOOT_BASE_URL}/todos" \
      -H "Content-Type: application/json" \
      -d '{"title":"seed-two","completed":true,"order":2}'
  )"
  ...
  SPRINGBOOT_SEED_ID_1="$(extract_json_id "${springboot_seed_1_json}")"
  SPRINGBOOT_SEED_ID_2="$(extract_json_id "${springboot_seed_2_json}")"

  if [[ -z "${EXPRESS_SEED_ID_1}" || -z "${EXPRESS_SEED_ID_2}" || -z "${SPRINGBOOT_SEED_ID_1}" || -z "${SPRINGBOOT_SEED_ID_2}" ]]; then
    echo "Failed to capture seeded IDs."
    exit 1
  fi

  echo "Express seeded IDs: ${EXPRESS_SEED_ID_1}, ${EXPRESS_SEED_ID_2}"
  echo "Spring Boot seeded IDs: ${SPRINGBOOT_SEED_ID_1}, ${SPRINGBOOT_SEED_ID_2}"
}

main() {
  print_section "Parity comparison"
  echo "Express base URL: ${EXPRESS_BASE_URL}"
  echo "Spring Boot base URL: ${SPRINGBOOT_BASE_URL}"
